...
4. **`nlp.py`** — reads `ships.csv`, parses the free-text `notes` field into structured `ShipEvent`s (via regex extracting trailing 4-digit years per clause) to derive an end year/reason per ship (e.g. "broken up", "sunk", "wrecked" ⇒ terminal event), and logs each parsed `Ship`. The former Tableau Hyper export (`tableausdk.HyperExtract`) was removed — this script no longer writes any file, it just parses and logs. Where this parsing logic moves next (a proper source-adapter module) is tracked separately. Still written in Python 2 syntax.
