...
- **Filter by ship class, not by date range.** Verified via a live SPARQL query that Wikidata classifies Royal Navy ships into rating-system types — `first-rate`, `second-rate`, `third-rate`, `fourth-rate`, `fifth-rate frigate`, `sixth-rate frigate`, `sloop-of-war`, `gun-brig`, etc. — distinct from later classes (`ironclad warship`, `battleship`, `cruiser`, `destroyer`, `submarine`, `Dreadnought`). Filtering by these rating classes (`wdt:P31`) + operator = Royal Navy (`wdt:P137 wd:Q172771`) captures the sailing-ship era on its own real domain boundary — the rating system itself stopped being used once ships moved to steam/ironclad — rather than an arbitrary hardcoded 1663–1860 range (which was never a real requirement, just an artifact of what happened to be on the Wikipedia page originally scraped).
